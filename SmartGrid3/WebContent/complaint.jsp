...
<tr>
<td align="left">	
					<a href="http://www.twtech.in:8080/SmartGridTest/New%20Logo%20-%20SmartGrid.png" target="_blank"><img src="http://www.twtech.in:8080/SmartGridTest/New%20Logo%20-%20SmartGrid.png" style="height: 50px; width: 100px;" title="SmartGrid" id="logo" alt="Logo" /></a>
</td>
</tr>
<tr>
...
Description=request.getParameter("describtion");
Entby="-";
//Name=fname+" "+lname;
Name=userid;
...
System.out.println("\n\n ismultipart-->>"+isMultipart);
	...
	while (itr.hasNext()) {
			...
			if(item.isFormField()) 
			{
						...
						try {
								...
								/* savedFile = new File("/usr/share/tomcat7/webapps/SmartGrid/Media/"+ComplaintDateTime+""+itemName); */
   ...
   try {
			...
			/* queryfor email */
			 String sql6="Select Email from db_gps.t_userdetails where FirstName like '"+fname+"' and LastName like '"+lname+"' and UserName like '"+userid+"'";
            ...
            System.out.println(ReportName+" "+Description+""+Name+""+Email2+""+ComplaintDateTime+""+ContactNumber);
			String compidck="select ComplaintID from db_gps.t_ComplaintDetail where ReportName='"+ReportName+"' AND Description='"+Description.replaceAll("'"," ")+"' AND EntBy='"+Name+"' AND Email1='"+Email2+"' AND ComplaintDateTime='"+ComplaintDateTime+"' AND ContactNumber='"+ContactNumber+"'";
			System.out.println("compidck====>"+compidck);
		...
		{      /*Query changes transporter removed*/
		    String sqlinsert="insert into t_ComplaintDetail(ReportName,Description,EntBy,Name,EmailID,Email1,ContactNumber,FileName,Category,Priority,ComplaintDateTime,ClosingComment,ClosedBy,Status,Website,PageName,Filecomment) "+
			" values('"+ReportName+"','"+Description.replaceAll("'"," ")+"','"+Name+"','"+Name+"','"+EmailID+"','"+Email2+"','"+ContactNumber+"','"+savefilestring+"','"+Category+"','"+Priority+"','"+ComplaintDateTime+"','"+ClosingComments+"','"+ClosedBy+"','"+Status+"','SmartGrid','"+PageName+"','"+comments.replaceAll("'"," ")+"')";	
		  ...
		  int i =0;
			...
			String sqlinsert1="insert into t_historyComplaintDetail(ReportName,Description,EntBy,Name,EmailID,Email1,ContactNumber,FileName,Category,Priority,ComplaintDateTime,ClosingComment,ClosedBy,Status,Website,PageName,Filecomment) "+
			" values('"+ReportName+"','"+Description.replaceAll("'"," ")+"','"+Name+"','"+Name+"','"+EmailID+"','"+Email2+"','"+ContactNumber+"','"+savefilestring+"','"+Category+"','"+Priority+"','"+ComplaintDateTime+"','"+ClosingComments+"','"+ClosedBy+"','"+Status+"','SmartGrid','"+PageName+"','"+comments.replaceAll("'"," ")+"')";	
			...
			String Data="<html><head><title>Mail Body</title></head><body><table border=^18^ bordercolor=^#E6E6FA^ align=^center^ width=^70%^><tr align=^center^><td align=^left^ style=^border: 0px;^ width=^100%^><table  style=^margin-left: 1em;margin-right: 1em;width: 100%;height: 100%;^><tr><td width=^100%^><a href=^http://www.twtech.in:8080/SmartGridTest/New%20Logo%20-%20SmartGrid.png^ target=^_blank^><img src=^http://www.twtech.in:8080/SmartGridTest/New%20Logo%20-%20SmartGrid.png^ style=^height: 80px; width: 100px;^ title=^SmartGrid^ id=^logo^ alt=^Logo^ /></a><br/><font style=^color: #D8D8D8^>_____________________________________________________________________________________________________________</font><body><br></br>"+Name+",<br><br>Thank you for contacting us. This is an automated response confirming the receipt of your complaint. One of our team members will get back to you as soon as possible. "
             ...
             +""+"</font></td></tr></table></div><br><br></div>"
             ...
             +"Phone: 9120 41214444 <br>Email:  avlsupport@Mobile-Eye.in </div><font style=^color: #D8D8D8^>_____________________________________________________________________________________________________________</font><br/><br/><br/></td></tr></table></td></tr></table></body></html>";
                         ...
                         String subjectstr="SmartGrid Complaint ID: "+complaintID+" , Received from  "+Name+""; 
			               String tocc="avlsupport@mobile-eye.in"; 
			                  /* String tocc="b_pakhale@transword-compressor.com"; */
...
try{
				...
				if(sendmsg>0)
				{
					...
					try
					{
						...
						/* sms="New Comp No: "+complaintID+",By: "+Name+",Contact: "+ContactNumber+",Website: SmartGrid,Rep Name: "+ReportName+",Cat: "+Category+",Pri: "+Priority+"";
				...
				e.printStackTrace();
								...
								<div
	                             ...
	                             <br></br>
								   ...
								   <td width="15%"><font size="2" face="Arial"> <b>Website</b></font></td><td width="25%"><font size="2" face="Arial"> SmartGrid</font></td></tr><tr>
